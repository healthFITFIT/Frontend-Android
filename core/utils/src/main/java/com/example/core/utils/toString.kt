...
fun getDateText(
    ...
    var text =
        if (dateTimeFormat.useMonthName){
    ...
    if (dateTimeFormat.includeDayOfWeek)
        text = when (dateTimeFormat.dateFormat) {
            DateFormat.YMD -> "$text $dayOfWeek"
            DateFormat.DMY -> "$dayOfWeek, $text"
            DateFormat.MDY -> "$dayOfWeek, $text"
        }
